...
import { FlaskConical, Gauge, LogOut, Moon, Sun, X } from 'lucide-react'
  ...
  return (
    <header className="h-[60px] shrink-0 flex items-center gap-3.5 px-5 border-b border-bs-border bg-bs-surface">
      ...
      <div className="ml-auto flex items-center gap-2">
        ...
        {op && (
          <span className="pill">
            <span className="w-6 h-6 rounded-full bg-bs-accent/15 text-bs-accent-text grid place-items-center text-[11px] font-bold">{initials(op.name)}</span>
            {shortName(op.name)}
          </span>
        )}
